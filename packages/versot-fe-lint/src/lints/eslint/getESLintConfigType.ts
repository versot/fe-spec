 ...
 * @param cwd
 * @param pkg
 * @returns versot-fe-eslint-config/index
 * @returns versot-fe-eslint-config/react
 * @returns versot-fe-eslint-config/typescript/index
 * @returns versot-fe-eslint-config/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
  ...
  return (
    'versot-fe-eslint-config/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '')
  );
}
